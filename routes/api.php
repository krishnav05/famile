...
->get();
    ...
    Route::post('getprofilechartdatadetail',function(Request $request){
     $data = ConvertedPrescription::where('profile_id',$request->profile_id)->select(DB::raw("SELECT * FROM converted_prescriptions"),DB::raw("MONTHNAME(consultation_date) as monthname"))->whereYear('consultation_date', $request->year)
->groupBy('monthname')
->get();
